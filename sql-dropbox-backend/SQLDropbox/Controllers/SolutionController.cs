...
public class SolutionController(AppDbContext db, SolutionService solutionService, SchemaService schemaService) : BaseController
{
    ...
    public async Task<ActionResult> SubmitSelectSolution([FromBody] SolutionDTO dto)
    {
        try
        {
            ...
            string queryToSave = dto.Query;

            try
            {
                string formattedQuery = _soS.FormatQuery(dto.Query);
                queryToSave = formattedQuery;
                ...
                if (!Valid)
                {
                    await _soS.RegisterUserSolution(formattedQuery, exercise, user, Message);
                ...
                {
                    queryOutput =
                        await _scS.ExecuteSelectOnSchemaAsync(exercise.Chapter.Schema.SchemaName, formattedQuery);
                }
                else
                {
                    ...
                    queryOutput = await _scS.ExecuteInsertUpdateDeleteOnSchemaAsync(exercise.Chapter.Schema.SchemaName,
                        formattedQuery, exercise.ValidationQuery);
                }
                ...
                {
                    await _soS.RegisterUserSolution(formattedQuery, exercise, user,
                        "This is wrong, an error should be saved (work in progress)!");
                    return Ok(new
                    {
                        message = "This is wrong, an error should be returned (work in progress)!",
            ...
            catch (PostgresException ex)
            {
                ...
                //        break;
                //}
                await _soS.RegisterUserSolution(queryToSave, exercise, user, ex.Message);
